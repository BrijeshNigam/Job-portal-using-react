...
router.post('/login', async(req, res) => {
    try {
        ...
        if (!user) return res.status(400).json({message: 'galat hai phir se try kre babu ji'});
        ...
        res.status(500).json({ message: 'dekh bhai server hai gussa abhi thodi der me aa phir', error: err.message });
    }
});
